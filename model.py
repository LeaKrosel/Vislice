...
class Igra:
    ...
    def pravilni_del_gesla(self):
        ...
        for crka in self.geslo:
            if crka in self.crke:
                # dodamo presledek, da je bolj pregledno, 
                # koliko crk je še potrebno ugotoviti
                delni += crka + " "
            else:
                delni += '_ '
        return delni[:-1]
    ...
    def nepravilni_ugibi(self):
        # s presledkom združi napačne črke
    ...
    def ugibaj(self, ugib):
        if len(ugib) > 1 or len(ugib) == 0:
            return VEC_KOT_CRKA
        crka = ugib.upper()
        if crka not in 'ABCČDEFGHIJKLMNOPRSŠTVUZŽXYWQĆĐ':
            return POSEBEN_ZNAK
        if crka in self.crke:
...
class Vislice:
    ...
    def ugibaj(self, id_igre, crka):
        ...
        # ne rabimo vrniti nič, funkcija vrne None
